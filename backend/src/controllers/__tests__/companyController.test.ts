...
describe("companyController", () => {
  ...
  describe("createCompany", () => {
    ...
    it("should create a company", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ data: company }));
    });

    it("should fail if executor missing", async () => {
      await createCompany({ body: { name: "TestCo" } } as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: false }));
    });

    it("should handle service errors", async () => {
      (CompanyService.createCompany as jest.Mock).mockRejectedValue(new Error("fail"));
      await createCompany({ body: reqBody } as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: false }));
    });
  });
  ...
  describe("getCompanies", () => {
    it("should return list of companies", async () => {
      ...
      (CompanyService.getAllCompanies as jest.Mock).mockResolvedValue(companies);

      await getCompanies({} as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ data: companies }));
    });

    it("should handle errors", async () => {
      (CompanyService.getAllCompanies as jest.Mock).mockRejectedValue(new Error("fail"));
      await getCompanies({} as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: false }));
    });
  });
  ...
  describe("getCompanyById", () => {
    it("should return company details", async () => {
      ...
      await getCompanyById({ params: { id: "1" } } as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ data: company }));
    });
    ...
    it("should handle service errors", async () => {
      (CompanyService.getCompanyById as jest.Mock).mockRejectedValue(new Error("fail"));
      await getCompanyById({ params: { id: "1" } } as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: false }));
    });
  });
  ...
  describe("updateCompany", () => {
    ...
    it("should fail if executor missing", async () => {
      await updateCompany({ params: { id: "1" }, body: { name: "UpdatedCo" } } as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: false }));
    });

    it("should handle service errors", async () => {
      (CompanyService.updateCompany as jest.Mock).mockRejectedValue(new Error("fail"));
      await updateCompany({ params: { id: "1" }, body: reqBody } as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: false }));
    });
  });
  ...
  describe("deleteCompany", () => {
    it("should delete company with executor in body", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ data: { message: "Company deleted successfully" } }));
    });
    ...
    it("should delete company with executor in header", async () => {
      ...
      await deleteCompany(
        { params: { id: "1" }, body: {}, headers: { "x-executor": "admin" } } as unknown as Request,
        res
      );
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ data: { message: "Company deleted successfully" } }));
    });
    ...
    it("should fail if executor missing", async () => {
      await deleteCompany({ params: { id: "1" }, body: {} } as unknown as Request, res);
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: false }));
    });

    it("should handle service errors", async () => {
      (CompanyService.deleteCompany as jest.Mock).mockRejectedValue(new Error("fail"));
